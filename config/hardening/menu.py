...
class Display_Menu:
		...
		self.network_ip = "10.8.0.2"
		self.network_netmask = "255.255.255.0"
		self.network_gateway = "10.8.0.1"
		self.network_dns1 = "10.8.195.10"
		self.network_dns2 = "10.24.195.10"
		...
		f.write('firewall-cmd --permanent --add-service=ssh --add-service=https\n')
	...
	def configure_system_profile(self,args):
		...
		if int(self.system_profile.get_active()) == 3:
			...
			# Firewall Configuration
			f.write('firewall-cmd --permanent --add-service=ssh --add-service=snmp\n')
			f.write('firewall-cmd --reload\n')
			# Runlevel Configuration
	...
	def check_network(self,widget,event=None):
		...
		if self.error == 0:
			...
			f.write('user --name=bdylan --groups=wheel,sshusers  --password='+str(self.password)+' --iscrypted \n')
			f.close()
